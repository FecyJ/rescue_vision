...
随后要求人工输入 `REPLAY`。当前 v3 日志是唯一支持格式；它在运动指纹中分别记录
线/角加减速度四项限制，旧 v1/v2 日志会被拒绝。
...
## 正式流程 Enter 发车闸门（真机未验证）

在架空轮完成首轮检查，再把车辆放到空旷场地并保持物理急停可立即触发。运行正式
`rescue-vision-match` 命令，按以下步骤验收：

1. `start_gate=ready` 必须在 UART 零速同步、有效编码器/IMU 遥测、相机/Hailo 首帧和
   预检全部成功后出现；提示前不得有非零轮速。
2. 在提示处等待至少 30 s，确认双轮目标和实际速度持续为零、25 Hz 零速心跳与 100 Hz
   遥测不中断，相机感知和可选预览/观察图传仍持续更新。
3. 输入任意非空文字并按 Enter，车辆不得启动；终端应提示直接按 Enter。随后只按 Enter，
   记录 `start_gate=released timestamp_ns` 到首条非零轮速命令及编码器实际起步的延迟，分别
   报告至少 20 次的 P50/P95/最大值。
4. 分别在等待期间锁存急停、断开相机/推理和制造 UART 同步丢失，确认程序拒绝发车并进入
   软刹车清理；恢复外设不能自动越过闸门，必须重新启动并再次人工按 Enter。
5. 对 `rescue-vision-match-nb`、`rescue-vision-match-cc`、策略入口和
   `rescue-vision-grab-transport` 各复测一次，确认共用运行时行为一致。

自动回归只验证放行边界与资源维护调用，不能证明 Raspberry Pi 调度延迟、STM32 实际响应
时间或轮胎起步延迟；这些结果在完成上述测量前均记为未验证。
...
1. 启动结束后有适合单绿时直接夹取；必须解团时车辆先真实停稳，连续有效轮编码器/IMU
   证据确认停车，零速命令不能替代它。
2. 近场准备器在抓取受阻后找到带可靠 K0 的接触核心，并检查整段危险目标、场界、安全区和
   机器人包络。确认前保存团内得分物块的类别、实测坐标与采集时间。
3. 当前核心通过 `match.breakup_confirmation_frames` 个不同有效帧后冻结计划，必要时按 IMU 对准；
   全程闭爪前进 0.5 m、停稳、后退 0.3 m、停稳，不出现张爪指令。
   期间遮挡外围物块并制造 tracker 换号，标记不能清空；后退后新观测应重新关联原得分目标，
   输出 `breakup_complete_replan_grasp_task` 并接回对准与夹取检查，不重新无目标扫描。
...
该入口复用当前 match，只替换开场动作。`match.nb_opening_actions` 的动作、速度和
夹爪配置以 [app README](../src/rescue_vision/app/README.md) 为权威，这里只列真机核对项。
先在物理急停与全程监督下运行：
...
1. 确认动作严格按 `nb_opening_actions` 的列表顺序执行；转向角为带符号 rad，直行
   距离为带符号 m，负距离表示倒车。
2. 每次动作变化都会打印 `nb_opening=action=...`，包含配置量、速度和实际进度；不再
   出现目标航点/位置误差校准日志。
3. 每个 `turn` 动作在完成其 IMU 相对角度前线速度必须为 0；完成后先按
   `nb_opening_settle_time_s` 保持零速，下一动作才允许执行。
4. `nb_opening_gripper_after_action` 指定的动作完成后张爪，后续动作持续保持配置的
   左右角度；倒车动作的线速度必须为负。
5. 人为制造航向不可信后，单次转向超过 `nb_opening_turn_timeout_s` 时打印
   `nb_opening_turn_timeout_stop` 并进入 `terminal_stop`。
...
3. 单个蓝块直取：确认 `transport_align_green` 阶段的对准行为与 2026-09-11 运行一致
   ...
   说明相位分派失效。
4. 全蓝目标团解团：该独立策略阶段仍不使用正式得分目标标记；共用执行器已改为闭爪定距
   前进 0.5 m、后退 0.3 m，需重新验收完整路径及实际位移。
...
一般阶段绿/黑收拢完成后，验证补夹扫描可接收全场目标，不因 `max_range_mm` 截断；候选按机器人
地面距离优先。确认一个补夹目标后人为放入更近目标，车辆仍保持原目标；遮挡或移除原目标后，
才允许按正常对准→接近→近场确认链接管下一个目标。记录原目标失观时间、接管目标、观测年龄、
确认进度和实际返程原因；纯逻辑测试不能替代带载留存和二次开爪真机验证。
...
按正式 match 入口使用当前实车配置与监督急停，保留控制周期、感知延迟和动作日志：

- 首次单绿阶段保留侧方不可达候选及正前方可达绿色，验证失败后换目标，不能不断重开近场会话。
- 已完成首趟后，在约 260～450 mm 放两绿、绿黑、两黑及三个绿黑组合，验证近场按容量开爪收拢；
  人为短暂遮挡使 tracker 重新编号，不能持续沿旧参考点接近。
- 单绿旁放蓝块，分别验证蓝块在实际扫掠外可单抓绿色、侵入车体/夹爪路径时拒绝；超过容量不得混夹。
- 安全区退出后视野为空/只有蓝块，验证一圈后换位；安全区、场界或蓝块阻挡当前方向时换方向，
  换位中新增危险侵入仍须退出当前路径。
- 记录有界状态推进时间、控制周期分位数、采集年龄与结果发布时间。无硬件 pytest 不替代以上实车验收。

### 远场解团对准与实际穿入回归

使用 `configs/runtime.match.yaml` 或 `configs/runtime.match_nb.yaml`，在现有受监督真机流程中
摆放 600–800 mm 外的侧偏绿黑混合团，加入外围成员遮挡，再测试浅黑核接触核心。
记录采集/发布/遥测时间、控制周期 P50/P95、冻结航向误差、编码器前后行程、标记物块位置与
刹车超调；确认旋转期间迟到的 K0 不使瞄准点漂移，连续前推跨过首次接触位置，达到
0.5 m 终点才停稳闭爪后退 0.3 m。超过固定动作接触范围的远场团必须先正常接近，不能盲推。
另测危险块侵入场界/安全区推移路径、遥测断档与超时同团重现。
自动回归采用 5/10 ms 控制轮询、300/600 ms 感知延迟和 250/400 ms 帧间隔；
它不能替代目标硬件端到端延迟、实际制动及解团效果验收，以上真机项目目前**未验证**。

靠边进展验收补充：以 2032 日志截图附近的 `(979,-158)` mm、约 `-69.3°` 朝向为
参考布置绿蓝混合团，实测 K0 和场地边界后验证：夹取受蓝块阻挡时，合法解团应在
对准/停稳/确认预算内实际前推；真实车体、夹爪或蓝块推移越界仍须拒绝并有效换位。
核对 `blocked_path` 与实测实体扫掠，不能只看等待原因。该布置的几何重建是合成回归，
现场边界误差、实际危险块位移和动作成功率仍**未验证**。

2240 日志修复追加验收：在场地一侧旋转观察远场单橙，记录采集年龄与当前 K0、确认搜索/换位能转入接近；
带一块物资扫描时让首个候选失观或受阻，确认失败区域换号后仍被排除，并在原扫描预算内接近其它近场/远场绿黑；
摆放中心间距略大于旧侧邻门限但实际张爪可覆盖的并列绿黑，确认等待局部确认后优先共同收拢，剩余容量为一时仍只选一块。
这些场景必须同时检查危险侵入拒绝和目标设备控制周期、采集到动作端到端延迟；当前未验证。

## 2315 抓取任务整链验收（未验证）

使用实际运行配置与已有监督急停流程运行正式match/NB，分别记录有/无预览、稀疏与密集布局。
流程与参数权威为[正式流程设计](../docs/正式流程设计.md#抓取任务与动作顺序)。

1. 单绿与单橙从远场进入近场，加入短时遮挡、换号及外围目标闪烁；检查任务身份、总截止时间连续。
2. 稳定单帧经过300–1500 ms感知/准备延迟后，应按当前真实静止证据提交动作，不追加锁组往返；
   记录采集、感知完成、准备完成、命令下发四个时间戳。
3. 抓取受阻时验证同场景安全恢复，记录前推/实际穿入/退离量；退离后出现
   `breakup_complete_replan_grasp_task`，用新几何再抓取，不整圈扫描。接触后目标移动/换号单独记录。
4. 无安全恢复、无改善、恢复次数耗尽或任务到期时，应发生实际换目标/经检查的换位；
   同位置、原地转向或新ID不能清除失败。复现场景143–148秒循环与track=52重复接管。
5. 增组扫掠合法时优先多抓，蓝块使增组不合法时仍抓合法单块；覆盖首趟、单橙、容量、
   已携带/已交付排除。带载补夹不得启动解团。
6. 在计划计算后移动机器人、制造遥测失效、核心缺失及危险侵入，验证旧场景不能提交；
   不把单帧漏检、rx_degraded或显示/日志异常单独作为停车理由。
7. 记录发现→首次有效动作、停稳→抓取提交、解团完成→再次抓取提交、实际抓获率、
   控制周期P50/P95/P99/最大值与 `stationary_reason`。危险类漏检/侵入及最大推移独立报告。
...
对应 `logs/match_20260913_1651.log` 的三个现象：解团只转向不推进、首轮不选近处孤立单绿、
看见黑核却反复 `search_cluster` 甚至 `relocate_forward`。软件改动与无硬件回归见
[正式流程设计](../docs/正式流程设计.md#解团恢复与重新抓取) 与
[后续优先级](../docs/后续优先级.md#20260913-1651-现场问题的修复验收未验证)。
现场日志判读基准：该场 611 s 内近场交接 72 次、带原因重选 41 次（38 次
`scene_evidence_unavailable`）、真实解团推挤仅 2 次，推挤前累计绝对转角 5.9 圈。

1. 正前方摆放密集绿黑团（含互相阻挡、开口不足的接触），记录从进入近场到第一次实际推挤的
   时间、转角与 `breakup_forward` 次数。修复前 5 份需要对准转向的接触计划全部被弃，
   应确认现在 `grasp_recovery_turn_frozen` 之后直接出现 `breakup_plan_frozen` 并前推，
   不回到 `transport_near_field_grasp` 重扫。**同时测量推挤次数上升后的实际穿入量、
   场界余量与双方安全区净空**——推挤变频繁是这次改动的预期副作用，必须单独验收。
2. 制造近场提交阶段的短暂丢帧（遮挡一帧、把 `near_field_grasp` 准备结果延迟到 800 ms 之后），
   确认已锁定成员的会话不再在 800 ms 处退出，而是在提交窗口内完成开爪；同时确认
   「完全没有方案」的路由仍在短窗口内带原因退出，不占用提交预算空等。
3. 复现 `scene_evidence_unavailable`：确认同一片区域先出现一次
   `near_field_reobserve_without_motion:...`（保留 `grasp_task` 与交接目标、不新增
   `_near_field_failures`），第二次失败才以 `near_field_route:reselect:...` 退出并记录失败。
   记录两次尝试之间的实际间隔，确认不再是 7～15 s 空转加 300 mm 前移；确认退出仍有界。
4. 首轮场景只放一个近处孤立绿块和一个远处绿块，读取日志新增的 `approach_seed=` 字段，
   逐项记录近处孤立单绿被淘汰的第一个门禁（或 `accepted`）。**在拿到这份日志前不改排序键**：
   现有证据只能证明「首轮最近单绿」规则位于不可达代码路径，不能证明是哪一条谓词淘汰了它。

## 夹爪颜色与运输调整验收（未验证）

使用 `configs/runtime.match.yaml`、固定相机和本地预览，核对内侧多边形不覆盖夹臂/车体。
分别测试单绿亮面/暗面、不同方向与交叉点画线、黑块与线相连、绿旁误入真实黑/橙/蓝、单橙旁混入其他颜色。
确认仅有效冲突触发开爪—后退250 mm—重选；危险类漏检单列，不用合成图证明现场召回。
记录 `gripper_color` 的原始黑色、暗面扣除、最终各色比例与采集年龄，实测机械开爪时间和后退制动距离。
另外验证2+1累计、3个跳过补夹、红/蓝方bbox偏左/偏右只见对侧两点仍推进、退区无安全区图也立即搜索。
Pi/Hailo控制周期与端到端延迟须在该负载下重新测量；已有pytest结果不能代替真机验收。

### 安全区两点标定时延复测

使用 `configs/runtime.match.yaml`，记录 d1 停车到 D2 路线启动的 P50/P95 时间及一次成功率。
分别覆盖 bbox 偏左只见 K0/K2、偏右只见 K0/K1、第三点闪烁、框完整但关键点暂缺。
完整 bbox 四边保留 12 px 且两点可靠时应无额外转向。
分别遮出左、右、上、下边及左右两边，即使三个关键点齐全也不能取样；检查完整框判定日志。单侧裁剪检查一次固定转角、两侧/上下裁剪检查一次 100 mm 后退；
停车后才能取样，记录编码器/IMU停稳起点、帧采集时间、结果发布时间与真实制动距离。
叠加 300–600 ms 感知延迟和 250–400 ms 帧间隔，检查重复旧图不计数、不延长等待，
运动、遥测断档/无效及危险状态仍拒绝对应动作。始终缺点或拟合失败应记录降级原因，
退出视觉纠偏后以航位规划 D2，单列此时的末端位置误差。以上真机指标未验证。

安全区校准预算回归（1605 日志后）：检查首个有效帧接近旧截止时，确认预算只启动一次；
第二个有效帧在确认截止当轮返回应先拟合，短暂漏一帧不清空仍有效的首帧。记录
`first_frame_deadline_ns`、`confirmation_deadline_ns`、确认进度和具体拒绝原因，
统计实际视觉校正成功率、降级率及投放横向误差。连续真实运动、无效遥测和裁剪框仍须拒绝。
此次软件修复不改变降级路线；目标硬件降级概率与左右投放精度尚未验证。
