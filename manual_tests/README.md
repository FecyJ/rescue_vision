...
该入口复用当前 match，只替换开场动作。航点、速度和夹爪角度以
[app README](../src/rescue_vision/app/README.md) 为权威，这里只列真机核对项。
先在物理急停与全程监督下运行：
...
1. 确认动作严格为：转向→前进到 `(100,800)`→张爪→转向→前进到 `(100,-900)`→
   保持朝向倒车到 `(100,0)`；倒车前不得出现第三次原地转向。
2. 每次阶段变化都会打印 `nb_opening=phase=... target=... position=... error=...`。
   三个航点到位时分别核对坐标误差；明显偏大时先区分车辆落点与航位推算漂移。
3. 两段前进中只能看到连续的 `nb_opening_to_first` / `nb_opening_to_second`，不得反复
   停车重新对准；行驶中的小角速度只用于保持起步时固定的航向。
4. 张爪后直到开场结束都保持配置的左右角度；倒车段线速度必须为负。
5. 每个航点到位后只有约 `nb_opening_settle_time_s` 的短暂零速动作切换停顿。
6. 人为制造航向不可信后，确认起步转向超过 `nb_opening_align_timeout_s` 时打印
...
记录采集/发布/遥测时间、控制周期 P50/P95、冻结航向误差、编码器前进行程、张爪位置与
刹车超调；确认旋转期间迟到的 K0 不使瞄准点漂移，连续前推跨过首次接触位置，达到
计划穿入终点才停稳张爪。另测危险块侵入场界/安全区推移路径、遥测断档与超时同团重现。
