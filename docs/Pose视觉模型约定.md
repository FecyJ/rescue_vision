...
- 当前相机/模型组合的 `GroundPoint.x` 存在实测 `210 mm` 前向偏差；目标和场地
...
- 四类目标的运行时任务类别以模型标签为唯一权威，HSV 只按该类别提取几何掩码，
  不覆盖类别、也不输出 `unknown` 任务类别；颜色不足或歧义只保留分割诊断，
  不产生质量门禁。HSV 掩码不可用时夹爪横向包络使用 bbox 底边；模型 K0
  缺失或低置信时，任务目标地面锚点使用 bbox 底边中点。
  场地两类不进入目标 HSV 分类。
