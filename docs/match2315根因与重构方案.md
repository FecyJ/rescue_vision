...
本文记录 `logs/match_20260912_2315.log` 的事故分析与重构进度。当前统一抓取任务上下文、首帧确认、异步结果消费、同场景恢复、解团回接及物理目标失败记忆已落地；最终验收与真机验证仍未完成。
...
当前已完成针对性回归（116 项）及全量自动测试（1311 通过）；NB 配置固定预期的 3 项既有失败仍保留，另有 2 项旧测试与“危险证据必须继续参与检查”的新契约不一致，已修正实现并待测试基线同步。外围闪烁、恢复耗尽、补夹计数连续性和超时重现等最终验收仍待补齐；真机控制周期、端到端延迟和实际抓取效果未验证。
