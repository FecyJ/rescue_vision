...
正式流程的当前搜索行为以[正式流程设计](docs/正式流程设计.md)为准：首轮受阻绿色会先尝试其它安全绿色，只有没有合法抓取方案才进入动态局部解团；解团采用“采集位姿补偿与 K0 射线对准→真实停稳→当前帧接触核心→一次安全检查→少量连续帧确认→冻结→前推/张爪/后退”闭环，退出后再恢复搜索或近场抓取。 接近、近场和动态解团使用物理场界并按实体包络扣一次余量，避免靠边动作因重复缩边被全部否决。一圈搜索预算耗尽时保留失败记忆：优先换其它合法目标，无合法目标则执行经过路径检查的有界换位。 当前视野为空或只剩蓝块也执行此退出策略。近场受阻换目标前保存交接目标的物理失败证据；远场目标丢失/重新编号时接管当前合法候选，绿黑成员在接近与近场选组间使用一致的共同收拢规则。
...
补夹候选为全场绿/黑，不受 `near_field_grasp.max_range_mm` 截断，按机器人地面距离优先；目标确认后保持同一目标，在目标失观或动作明确失败后才接管下一个补夹目标；失败时保留物理失败记录并继续原扫描预算，并重新走正常对准、接近和近场确认链。
...
- v3 任务目标 K0 是底面几何中心投影，`ground_point` 直接表达该中心；不会用检测框中心或底边兜底。旧 `TargetGroundGeometryEstimator` 不在 v3 主链路中。
...
| `rescue-vision-match-nb` | 当前正式流程的开场变体；依次执行转向→前进到 `(100,800)`→张爪→转向→前进到 `(100,-900)`→保持朝向倒车到 `(100,0)`，其余复用正式流程 |
