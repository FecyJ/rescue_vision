...
`runtime.yaml` 配置比赛时长、无运动超时、对手接触超时和一般阶段目标优先级。前三项默认分别为官方初赛的 180 秒、15 秒和 10 秒；现场正式文件变化时必须同步核对[赛题约束与视觉需求](../../../docs/赛题约束与视觉需求.md)，不能只调配置而不记录规则来源。危险目标的路径避障由应用规划器按实际走廊判断，状态机不再使用固定全向距离触发避让。

持续推动已确认危险目标仍采用项目保守的 `TerminationReason.DANGER_ENGAGED`；解团阶段无目标锁定的短暂接触不视为转运，也不会仅因危险目标出现在视野或机器人附近而终止。
