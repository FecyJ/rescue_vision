...
解团终点已锁存，里程回落不会重新驱动；停稳失败超过截止时间后以
`breakup_stop_unconfirmed` 保持可恢复制动，有效静止证据恢复后自动推进，不锁死比赛流程。
正式 `match` 的动作超时及近场编码器失败均不再进入永久停车态：绝对点动作从当前位姿重试，
近场动作失败则退出本次尝试并重选。非接触动作不再执行目标、场界或安全区的通用扫掠碰撞
门禁；D1、D2、重定位、转向及显式倒车均直接采用运动控制器输出，持续可见的物块不得阻塞
动作。只有赛前检查失败、明确急停/事故/失控等直接安全终止，以及完成规定运输次数，仍保持
锁存停车。
...
`StationaryMotionEvidence` 观察到连续的新遥测停稳，才会切换或张爪。越过目标后直接
