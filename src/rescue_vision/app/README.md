...
| `rescue-vision-gripper-width` | 有地面标定时每秒测量居中物块并调整双舵机夹爪 |

## `rescue-vision-gripper-width` 使用方法

该入口复用配置中的相机、内参、地面映射、Hailo v3 检测和 ROI HSV 分割，并创建
运动控制器与 UART 夹爪执行器，但不会驱动车轮。每秒从最新有效快照中测量所有已通过
颜色掩码有效的物块，不按颜色类别筛选；颜色只用于提取前景边界，不参与目标选择。目标 K0 的地面投影 `y` 必须满足
`-5 < y < 5 mm`。若有多个候选，选择机器人坐标系中 `x` 最小、即最近的物块，计算
左右地面 `y` 边界和最大宽度，加 4 mm 后使用夹爪运动学反解双舵机绝对角度并下发。
每个 1 秒周期默认连续采集 5 个感知快照；目标丢失的快照保留在连续帧计数中，但不进入
平均值。每个采样帧和周期汇总都会打印有效帧数、宽度、目标开口、舵机角度以及不受
中心 `y` 门限影响的最前物块位置；实时旁路若出现源帧序号跳跃，会记录诊断但仍保留
这 5 个已收到的感知快照。
...
按 `q` 或 `Esc` 退出预览，`Ctrl+C` 退出命令。使用 `--once` 可在第一次成功下发角度后
退出；`--sample-frames` 可调整每个周期的连续帧数，默认值为 5。运行前必须确认配置中的相机内参、地面映射、Hailo 模型、UART 和
`motion.gripper` 开闭端点与当前车辆匹配。程序要求物理急停和全程监督；目标过宽、
舵机方向不符或固件夹爪输出不可用时跳过该次指令或退出，不会强行截断角度。
预览使用保持宽高比的可缩放窗口（大帧默认压到约 `1280×720`），画面会叠加最近一次
平均测量的中心 `y`、左右边界 `y`、宽度、目标开口和舵机角度。
