...
- 默认主流程 tracker 对两个明确且不同的规则类别保持硬隔离；近场
  `GraspTargetTracker` 会显式开启类别软约束，让 K0 空间连续性优先、类别
  只作为二级代价，同时在上层保留危险/质量历史；
- 关联使用确定性一对一最小代价分配，不按检测列表顺序贪心抢占近邻轨迹；
- 近场 tracker 还保留低置信轨迹到正常 coast 时限，低置信目标仍作为不可选
  障碍使用，因此不会因阈值瞬时跌落而生成每帧临时 ID；
...
- 当前使用确定性 K0/IoU 门控和 Hungarian 一对一分配，不声称达到正式 MOT 指标；
