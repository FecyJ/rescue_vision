...
把远程调试意图转换为底盘动作的实现位于相邻 `motion` 包。
...
需要分别验证真实相机图传或远程采集命令时，使用
[`manual_tests/README.md`](../../../manual_tests/README.md) 中的
`remote_video.py` 和 `remote_capture.py`。两者是单客户端真机人工测试入口，
不是可被生产应用导入的发布器。

## 当前边界

当前没有正式应用使用的会话/车辆/采集状态发布器、心跳调度、看门狗、急停
恢复、电脑操控界面或运动脚本。小车命令编码和远程意图到底盘的适配位于
相邻 [`motion`](../motion/README.md) 包。地图渲染
入口也尚未实现；人工测试目录中的相机图传和采集命令脚本只用于单链路真机
联调，不能视为正式发布器或应用入口。现有观察 dataclass 只冻结协议 schema。
它们属于 P1 后续阶段，不得把本模块描述为已完成远程驾驶、图传应用或整车
控制。独立电脑端不得导入本包；线级帧格式、消息 schema 和跨项目验收见
